...
func TestROSAOCMRoleConfigRoleConflict(t *testing.T) {
	...
	h.g.Eventually(func(g Gomega) {
		...
		g.Expect(err).To(HaveOccurred())
		g.Expect(err.Error()).To(ContainSubstring("already has a different OCM role linked"))
		...
		g.Expect(readyCondition.Message).To(ContainSubstring("already has a different OCM role linked"))
	}).WithTimeout(30 * time.Second).Should(Succeed())
}

// TestROSAOCMRoleConfigNoConsoleTagPolicyMismatch tests that reconciliation fails when role has no-console tag but wrong policy.
func TestROSAOCMRoleConfigNoConsoleTagPolicyMismatch(t *testing.T) {
	...
	// Test: Should fail with policy mismatch error
	req := ctrl.Request{NamespacedName: types.NamespacedName{Name: h.ocmRoleConfig.Name}}
	h.g.Eventually(func(g Gomega) {
		_, err := reconciler.Reconcile(h.testCtx, req)

		g.Expect(err).To(HaveOccurred())
		g.Expect(err.Error()).To(ContainSubstring("the role has the no-console tag but the no-console policy is not attached"))
