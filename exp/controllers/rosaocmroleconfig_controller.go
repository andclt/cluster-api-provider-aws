...
func (r *ROSAOCMRoleConfigReconciler) reconcileOCMRole(_ context.Context, scope *scope.ROSAOCMRoleConfigScope, rt *rosacli.Runtime) (ctrl.Result, error) {
	...
	roleARN, orgID, created, err := ocmrole.GetOrCreateOCMRole(
		rt,
		&scope.Logger,
	...
	// Update status with role ARN and organization ID
	...
	// Check for role conflict (different role already linked for this AWS account)
	if existsOnOCM && linkedARN != roleARN {
		v1beta1conditions.MarkFalse(ocmRoleConfig, expinfrav1.ROSAOCMRoleConfigReadyCondition,
			expinfrav1.ROSAOCMRoleConfigReconciliationFailedReason, clusterv1beta1.ConditionSeverityError,
			"AWS account %s already has a different OCM role linked: %s (expected: %s)",
			rt.Creator.AccountID, linkedARN, roleARN)
		return ctrl.Result{}, fmt.Errorf("AWS account %s already has a different OCM role linked: %s (expected: %s)",
			rt.Creator.AccountID, linkedARN, roleARN)
	}
	...
	if !existsOnOCM {
		...
		linked, err := rt.OCMClient.LinkOrgToRole(orgID, roleARN)
		if err != nil {
			v1beta1conditions.MarkFalse(ocmRoleConfig, expinfrav1.ROSAOCMRoleConfigReadyCondition,
				expinfrav1.ROSAOCMRoleConfigReconciliationFailedReason, clusterv1beta1.ConditionSeverityError,
		...
		scope.Debug("OCM role already linked to organization", "roleARN", roleARN, "orgID", orgID)
	}
