...
func GetOrCreateOCMRole(
	...
	roleName := awshelpers.GetOCMRoleName(prefix, awshelpers.OCMRoleType, externalID)
...
func checkRoleExists(awsClient rosaaws.Client, creator *rosaaws.Creator, log logger.Wrapper, roleName string, profile RoleProfile, rolePath string) (string, bool, error) {
	...
	case ProfileAdmin:
		...
		expectedAdminPolicyARN := awshelpers.GetAdminPolicyARN(partition, accountID, roleName, rolePath)
...
func createRole(awsClient rosaaws.Client, creator *rosaaws.Creator, log logger.Wrapper, prefix string, roleName string, rolePath string,
	...
	if profile != ProfileNoConsole {
		policyARN = awshelpers.GetPolicyArnWithSuffix(partition, accountID, roleName, rolePath)
	}

	// Build trust policy
	filename := fmt.Sprintf("sts_%s_trust_policy", awshelpers.OCMRolePolicyFile)
	policyDetail := awshelpers.GetPolicyDetails(policies, filename)
	policy := awshelpers.InterpolatePolicyDocument(partition, policyDetail, map[string]string{
		"partition":           partition,
		"aws_account_id":      awshelpers.GetJumpAccount(env),
	...
	case ProfileAdmin:
		...
		policyARN = awshelpers.GetAdminPolicyARN(partition, accountID, roleName, rolePath)
